...
## Round 4 — Header pattern, hero sizing, home enquiry form (2026-04-29)

- **Header pattern: transparent over hero, solid cream on scroll.** Round 3's padding-only fix didn't address the underlying issue Jack flagged — the cream header had no contrast against the viewport edge so it looked cut off. Going transparent over the hero lets the hero own the full screen and matches the premium pattern (NSP, Bosland). Logo + nav links + nav-toggle are scoped to cream colour via `:not(.scrolled)` rules; on scroll past 50px they revert to default colours under a solid cream bg. Padding compacts 28px → 18px when scrolled to feel responsive and less obstructive.
- **Hero sizing.** Home `.hero` raised from 90vh to `100dvh` (with `100vh` fallback). Inner pages `.hero-inner` raised from 60vh to 70vh. Contact's `.hero-compact` 50vh override removed (now matches inner at 70vh) — the 50vh value was sized for the old solid header pattern and felt too short under the transparent header. Removed the pre-existing `margin-top: -84px` / `padding-top: 84px` offset on `.hero` since the hero now starts at body top zero. `main { padding-top: 100px }` zeroed since hero sits outside `<main>` and there is no longer a fixed-header buffer to clear.
- **Home enquiry form.** Added between Why-Choose split and final CTA band. Two-column cream-dark layout: contextual copy + phone/email/hours rows on the left, white form card with shadow on the right. Form action uses `FORMSPREE_ID_HERE` placeholder (same as contact form) — backend wiring is a future round. Existing JS handler in `main.js` already uses `querySelectorAll('form[data-formspree]')` so both forms are handled generically without a JS change.
- **Footer separation:** `.site-footer` gets `border-top: 1px solid rgba(247, 250, 252, 0.08)` to create a subtle off-white hairline between the deep-teal CTA band and the deepest-teal footer.
